...
export default function Header({ toggleSidebar, title, isGuest, guestId }) {
  ...
  return (
    // زيادة Z-index للهيدر
    <div className="h-16 flex items-center justify-between px-4 sticky top-0 bg-[#f0f4f9]/90 backdrop-blur z-50">
        ...
        <div className="relative" ref={menuRef}>
            <button 
                ...
                className="flex items-center gap-2 hover:bg-gray-200 p-1 rounded-full transition-colors"
            >
                <UserCircleIcon className="w-9 h-9 text-gray-400" />
            </button>

            {isMenuOpen && (
                // إصلاح التموضع والظل والـ Z-Index للقائمة
                <div className="absolute right-0 mt-2 w-80 bg-[#eef2f6] rounded-[28px] shadow-2xl border border-gray-200 p-4 z-[100] animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                    ...
                    <div className="bg-white rounded-[20px] p-4 shadow-sm text-center mb-2">
                        ...
                        <div className="w-16 h-16 bg-gray-100 rounded-full mx-auto mb-4 flex items-center justify-center text-3xl">
                             👤
                        </div>
